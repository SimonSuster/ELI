...
flags.DEFINE_string(
    "bert_config_file", os.path.join(config.BIO_BERT_DIR, 'bert_config.json'),
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string(
    "init_checkpoint", os.path.join(config.BIO_BERT_DIR, 'bert_model.ckpt'),
    "Initial checkpoint (usually from a pre-trained BERT model).")

flags.DEFINE_string("vocab_file", os.path.join(config.BIO_BERT_DIR, 'vocab.txt'),
...
class InputExample(object):
  ...
  def __init__(self, guid, text_a, text_b=None, label=None, extra = None):
    ...
    self.text_b = text_b
    self.label = label
    self.extra = extra # o ya just stuff everything in here
...
class IcoABProcessor(DataProcessor):
...
class ExtractorProcessor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      guid = "%s-%s" % (set_type, i)
      pmid, sidx, fidx, i, c, o, s = map(tokenization.convert_to_unicode, line)
      label  = '0'
      text_a = s
      text_b = '{}; {}; {};'.format(i, c, o)
      examples.append(
